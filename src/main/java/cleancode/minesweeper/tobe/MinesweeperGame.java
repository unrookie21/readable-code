...
public class MinesweeperGame {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            landMines[row][col] = true; // 지뢰를 10개 설정하는 작업으로 추정된다.
        }
        for (int row = 0; row < 8; row++) {
            for (int col = 0; col < 10; col++) {
                int count = 0;
                if (!landMines[row][col]) { // 지뢰가 아닌 칸에서, 주변 8칸에 지뢰가 몇개있는지 check 한다.
        ...
        while (true) {
            ...
            if (userActionInput.equals("2")) {
                ...
                if (landMines[selectedRowIndex][selectedColIndex]) { // 사용자 선택이 지뢰 셀이라면
                    board[selectedRowIndex][selectedColIndex] = "☼";
                    gameStatus = -1;
                    continue;
                } else { // 지뢰셀이 아니라 정상셀을 여는 경우
